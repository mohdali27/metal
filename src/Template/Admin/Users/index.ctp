            ...
            <div class="panel panel-transparent">
              <div class="panel-heading"><?= $this->Flash->render() ?>
                <div class="row">
					...
					<form id="showPages" method="post" action="<?=HTTP_ROOT?>admin/users/index<?=$action?>" >
                    ...
                    <div class="col-sm-7" style="margin-top:0px;">
                        <div class="collection-search">
                            ...
                            <div class="input-group ">
								<form method="post" action="<?=HTTP_ROOT?>admin/users/index?limit=<?=$limit?>">
								...
								<select class="form-control input-sm page-limit inner-search" aria-controls="basic-usage" name="searchby" style="width:140px;margin-right:10px" id="searchtype">
									  ...
									  <option value="last_name" <?php  if($searchby=='role'){?>selected="selected"<?php }?>>Last Name</option>
								 </select>
									<input type="text" class="input-sm search-text" placeholder="Search..." name="search" id="search-text" value="<?=@$search?>" >
									<input type="submit" value="Submit">
                                    <a class="" href="<?=HTTP_ROOT?>admin/users/index"><span class="bold">Reset</span>
                                    </a>
								</form>
                            </div>
                        </div>
					...
					<div class="btn-group pull-right m-b-10" style="margin-top:20px;">
                     <a href="<?=HTTP_ROOT?>admin/users/adduser" class="btn btn-primary btn-cons m-b-10"><i class="pg-form"></i> <span class="bold">Add User</span>
                    </a>
                </div>
                 </div>
              ...
              <div class="panel-body">
				...
				<form method="post" id="mainform" action="<?= HTTP_ROOT?>admin/users/delete">
                  <table class="table table-hover" id="basicTable">
                    <thead>
                      <tr>
                        ...
                        <th style="width:15%"><?= $this->Paginator->sort('username', 'Username');?></th>
                        <th style="width:15%"><?= $this->Paginator->sort('email', 'email');?></th>
                        <th style="width:15%"><?= $this->Paginator->sort('mobile', 'mobile');?></th>
					...
					<?php 
					 ...
					 ?>
                      <tr>
                        ...
                        <td class="v-align-middle">
						  <a href="<?=HTTP_ROOT?>admin/users/edituser/<?= $value->id?>" title="Edit User">
							<span class="glyphicon glyphicon-pencil"></span>
						  </a>
                          <a href="<?=HTTP_ROOT?>admin/users/delete/<?= base64_encode($value->id)?>" title="Delete Event" onclick="if(!confirm('Do you want to delete this record?')){return false;}">
