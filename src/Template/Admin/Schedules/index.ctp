            ...
            <div class="panel panel-transparent">
              <div class="panel-heading"><?= $this->Flash->render() ?>
                <div class="row">
					...
					<form id="showPages" method="post" action="<?=HTTP_ROOT?>admin/schedules/index<?=$action?>" >
                    ...
                    <div class="col-sm-7" style="margin-top:0px;">
                        <div class="collection-search">
                            ...
                            <div class="input-group ">
								<form method="post" action="<?=HTTP_ROOT?>admin/schedules/index?limit=<?=$limit?>">
								...
								<select class="form-control input-sm page-limit inner-search" aria-controls="basic-usage" name="searchby" style="width:140px;margin-right:10px" id="searchtype">
									  <option value="event_name" <?php  if($searchby=='event_name'){?>selected="selected"<?php }?>>Event Name</option>
                  ...
                  <a class="" href="<?=HTTP_ROOT?>admin/schedules/index"><span class="bold btn btn-primary m-l-10">Reset</span></a>
								</form>
                            </div>
                        </div>
                    </div>
					<div class="btn-group pull-right m-b-10" style="margin-top:20px;">
                     <a href="<?=HTTP_ROOT?>admin/schedules/addschedule" class="btn btn-primary btn-cons m-b-10"><i class="pg-form"></i> <span class="bold">Add Schedule</span>
                    </a>
                </div>
                 </div>
              ...
              <div class="panel-body">
				...
				<form method="post" id="mainform" action="<?= HTTP_ROOT?>admin/schedules/delete">
                  <table class="table table-hover" id="basicTable">
                    <thead>
                      <tr>
                        ...
                        <th style="width:15%"><?= $this->Paginator->sort('sch_date', 'Date');?></th>
                        <th style="width:15%"><?= $this->Paginator->sort('sch_timing', 'Timing');?></th>
					...
					<?php 
					 ...
					 ?>
                      <tr>
                        ...
                        <td class="v-align-middle">
                          <p><?=$this->Mystring->turncate($value->sch_description)?></p>
                        </td>
                        <td class="v-align-middle">
                          <p><?=$value->sch_date?></p>
                        </td>
                        <td class="v-align-middle">
                          <p><?=$value->sch_timing?></p>
                        </td>
                        <td class="v-align-middle">
						  <a href="<?=HTTP_ROOT?>admin/schedules/editschedule/<?= $value->id?>" title="Edit Schedules">
							<span class="glyphicon glyphicon-pencil"></span>
						  </a>
                          <a href="<?=HTTP_ROOT?>admin/schedules/delete/<?= base64_encode($value->id)?>" title="Delete Event" onclick="if(!confirm('Do you want to delete this record?')){return false;}">
